...
- **OpenAI pricing is booked from usage and learned for previews** (`server/src/providers/openaiPricing.ts`). Every response's `usage` tokens x published rates ($5 text in, $8 image in, $30 image out per 1M) is the exact cost booked to the ledger. There is no published per-image table for 2.5, so previews come from a price book learned from those bills per model x op x canvas x quality (`library/image-costs.json`), seeded from the gpt-image-2 table (low/medium/high $0.005/$0.041/$0.165 tall/wide, $0.006/$0.053/$0.211 square — a square costs MORE) plus a 1¢ reference-image seed for edits. The client has **no price table of its own**: `/api/health` serves `prices`, and pickers show `~$` until a tier has been billed once.
