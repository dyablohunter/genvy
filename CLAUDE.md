...
- **OpenAI gpt-image** (never gpt-image-1; there is no bare `gpt-image-2.5` model id) — the default provider: native alpha, multi-reference edits, good instruction-following. **The user picks the model per operation** from `OPENAI_IMAGE_MODELS` (`server/src/providers/openai.ts`): **`gpt-image-2.5-flare`** (fast), **`gpt-image-2.5-sunburst`** (tighter subject preservation on edits) and **`gpt-image-2`**, all at identical token rates. Each model has its own quality tiers — 2.5 offers low/medium/high/xhigh/max, gpt-image-2 low/medium/high — and the server clamps a tier the picked model lacks (`clampQuality`). With no pick, flare generates (`OPENAI_IMAGE_MODEL`) and sunburst edits (`OPENAI_EDIT_MODEL`). The pick rides requests as `modelFamily` and resolves via `provider.resolveModel(op, pick)`. Every picker (Sprite Forge blueprint, anchor chain, ✎/↻ modal, animation forge; World Maker scene panel, tileset and panel-modify modal) lays out **PROVIDER 60% · QUALITY 40%** on one line with MODEL below (the anchor chain, which uses the blueprint's provider, pairs MODEL 60% · QUALITY 40%), and QUALITY refills with the picked model's tiers and prices (`fillModelOptions` / `fillQualityOptions` in `client/src/hud/imageModels.ts`). Busy labels and timing buckets name the model that actually runs — never "GPT-IMAGE-2.5" alone, never one bucket for two models.
- **OpenAI pricing is booked from usage and learned for previews** (`server/src/providers/openaiPricing.ts`). Every response's `usage` tokens x published rates ($5 text in, $8 image in, $30 image out per 1M) is the exact cost booked to the ledger. Previews come from a price book learned from those bills per model x op x canvas x quality (`library/image-costs.json`). Until a bucket is billed, its seed is the **exact output cost** from OpenAI's calculator formula (`outputTokens`: `ceil(q * round(q*short/long) * (2e6 + w*h) / 4e6)`, q = 16/24/48/64/96 for 2.5 low/medium/high/xhigh/max — verified against OpenAI's calculator at 1536x1024: 158/343/1372/2459/5488 tokens = $0.00474/$0.01029/$0.04116/$0.07377/$0.16464, and low 196 tokens/$0.00588 at 1024x1024; a square costs MORE) plus the input cost: **measured** from real bills of the same op (bill minus formula output, averaged across every model and tier — input is billed alike whatever renders the output; e.g. LOW edits ≈ $0.0047 output + $0.0094 input = $0.014), and only guessed when no bill of that op exists yet (prompt text, ~130–960 tokens, plus a 1¢ reference-image allowance for edits — which overstated real edits by ~30%). 2.5 **renamed the tiers**: its `high` spends what gpt-image-2 `medium` did, and gpt-image-2 `high` is now `max`. Genvy offers all five under OpenAI's names (`IMAGE_QUALITY_TIERS` in `shared/src/api.ts`) — never relabel them (no MID). The client has **no price table of its own** and nothing hard-codes a price: every label reads `prices` from `/api/health`, showing `~$` until a tier has been billed once.
...
3. key its bucket by operation + shape **+ provider + the picked model** (`anim:openai:gpt-image-2.5-sunburst:8:std`, `tileset:image:openai:gpt-image-2`) via `timingTag(provider, op, modelFamily)` from `client/src/hud/imageModels.ts` — never by asset id, or the average never converges; models differ ~2x in latency, so no two share a bucket. Unlearned first runs pace with `scaleFallback(ms, provider, op, modelFamily)` (fallbacks were tuned on gpt-image-2; flare x0.5).

Keeping this accurate and informative is an ongoing job, not a finished feature: the **`progress-feedback` skill** (`.claude/skills/progress-feedback/`) carries the contract, a checklist, and a ranked list of improvement ideas (server-driven SSE stages, per-model buckets beyond OpenAI, retry-aware estimates, variance-based pacing, stall detection, cost preview). Load it whenever you touch an AI call or the busy indicator, and propose one concrete improvement.
...
A **level** is whichever layers it happens to have — a painted backdrop, a tile grid, gameplay zones, or any combination — so the editor never asks "what type of level is this?". There is no level TYPE at all, and nothing hybrid or mixed to choose: step 1 asks only what you are generating right now (**SCENE** or **TILESET**) and shows that generator's form; step 2 gives one toolset aimed at a layer, with layers the level does not have simply not offered. The layer row offers `TILES · ZONES` — the two a brush lands on; the backdrop is a layer of the level but is painted by the model in step 1, so EDIT CONCEPT is the way back to it, not a button pretending strokes apply. Coming back to step 1 to add the other layer is ordinary, not a mode switch.
