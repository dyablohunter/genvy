...
- **OpenAI gpt-image-2.5** family (never gpt-image-1 or -2; there is no bare `gpt-image-2.5` model id) — the default: native alpha, multi-reference edits, good instruction-following. Two variants at identical token rates, split by operation: **`gpt-image-2.5-flare`** (fast) runs text-to-image (`OPENAI_IMAGE_MODEL`: anchor candidates, tile grids, scenes) and **`gpt-image-2.5-sunburst`** (tighter subject preservation) runs every reference edit (`OPENAI_EDIT_MODEL`: anchor turns, animation sheets, frame repair, panel edits). The provider exposes `modelIds`, and busy labels and timing buckets name the model that runs — never "GPT-IMAGE-2.5" alone, never one bucket for both.
...
3. key its bucket by operation + shape **+ provider/model** (`anim:openai:gpt-image-2.5-sunburst:8:std`, `tileset:image:openai:gpt-image-2.5-flare`) via `timingTag(provider, op)` from `client/src/hud/imageModels.ts` — never by asset id, or the average never converges; flare and sunburst differ ~2x in latency, so they never share a bucket. Unlearned first runs pace with `scaleFallback` (fallbacks were tuned on gpt-image-2; flare x0.5).
